#!/usr/bin/env python3
"""
Nuclear fix — removes any broken catalogue entries and reinserts cleanly.
Run from blackbook/final:
  python3 patch_nuclear.py
"""
import re
...
# Step 1: Remove everything between KNI-M-DGN-IV06 line and defaultGifts
# This strips out any broken/partial catalogue insertions
pattern = r"(sku:'KNI-M-DGN-IV06'[^\n]*\n\s*\],\s*\n).*?(\n// Default gift set)"
match = re.search(pattern, c, re.DOTALL)

if not match:
    print("ERROR: Could not find anchor region. Showing context around KNI-M-DGN-IV06:")
    idx = c.find('KNI-M-DGN-IV06')
    print(repr(c[idx:idx+300]))
    exit()

print(f"Found broken region: chars {match.start(2)-50} to {match.start(2)}")
print(f"Removing {len(match.group())} chars of broken content")
...
# Replace the entire broken region
c = c[:match.start(2)] + clean_catalogues + c[match.start(2)+1:]  # +1 to skip the \n

with open(HTML_FILE, "w", encoding="utf-8") as f:
    f.write(c)

print("Done — all catalogues cleanly inserted.")
print("\nNow push:")
print("  git add index.html && git commit -m 'fix: clean catalogue insertion' && git push")
