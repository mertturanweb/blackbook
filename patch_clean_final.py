...
# Remove EVERYTHING between KNI-M-DGN-IV06's line-end and "const defaultGifts"
# by finding both anchors and slicing

start_anchor = "KNI-M-DGN-IV06"
end_anchor = "const defaultGifts"

i_start = c.find(start_anchor)
i_end = c.find(end_anchor)

if i_start == -1 or i_end == -1:
    print("ERROR: anchors not found")
    exit()

# Find end of the KNI line (the },\n  ],\n part)
line_end = c.find('\n', i_start)          # end of G1010F line
array_close = c.find('\n', line_end + 1) # end of   ],

# Everything from array_close to i_end gets replaced with clean catalogues
print(f"Cutting from char {array_close} to {i_end}")
print(f"Removing: {repr(c[array_close:array_close+80])}")
...
c = c[:array_close] + replacement + c[i_end:]

with open(HTML_FILE, "w", encoding="utf-8") as f:
    f.write(c)

print("Done — duplicates removed, clean catalogues inserted.")
print("\nPush:")
print("  git add index.html && git commit -m 'fix: remove duplicate catalogues' && git push")
