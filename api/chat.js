export default async function handler(req, res) {
  ...
  // ── CATALOG FETCH (no client required) ───────────────────────────────────────
  if (type === 'catalog') {
    try {
...
- Output only the message text, nothing else`;
  ...
  function buildGiftsPrompt() {
    ...
    const spendGuide = {
      'VVIC':     'Prioritise items above €3,000. Mix statement pieces (bags, coats, fine jewellery) with one or two refined accessories. No item below €500 unless it is an exceptional personal match.',
      'VIC':      'Focus on €1,500–€6,000 range. One hero piece above €3,000 with complementary items at €1,000–€2,500.',
      'Platinum': 'Sweet spot is €800–€3,500. One elevated piece if the occasion warrants it.',
      'Gold':     '€500–€2,500 range. Aspirational but not overextended for this relationship stage.',
      'Silver':   '€200–€1,500. Keep selections achievable and relevant to where this relationship currently is.'
    }[c.tier] || 'Mix price points thoughtfully based on the occasion and relationship stage.';
...
- Gender: match client gender — W items for women, M items for men, U is fine for either
- Occasion calibration: anniversary → more elevated/romantic; birthday → personal/celebratory; evergreen → versatile
- Shoe size: only recommend footwear if shoe size is on file
- Stock: avoid items with stock of 1 unless exceptional match
- Last purchase: do not repeat the same category twice in a row if avoidable

REASON WRITING:
- Each reason must be 1–2 sentences
- Reference a specific profile detail (colour preference, location, occasion, partner, size, LTV)
